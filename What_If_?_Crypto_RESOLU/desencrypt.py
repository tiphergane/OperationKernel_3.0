...
class Encryptor(object):
    ...
    def decrypt(self, ciphertext):
        ...
        for i in range(0, len(ciphertext), 16):
            ...
            # ? ciphertext[i:i+16] est connue (c'est la ligne chiffrée)

            # prendre a partire de la deuxieme ligne
            encrypted += bytes(self.xor(ciphertext[i:i+16], encryptedBlock))
            ctr += 1
        return encrypted    
...
def parse_args():
    ...
    parser.add_argument('file', help='file path to decrypt')
    parser.add_argument('passphrase', help='Passphrase used to decrypt file')
...
if __name__ == '__main__':
    ...
    ciphertext = open(args.file, 'rb')
    ctr = ciphertext.read(16)
    finalCipertext = ciphertext.read()
    passphrase = args.passphrase
    plaintext = Encryptor(passphrase).decrypt(finalCipertext)
    if args.output_file:
        ...
        print('Your file after encryption is', plaintext.hex())
