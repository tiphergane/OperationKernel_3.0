...
class Encryptor(object):
    ...
    def encryptBlock(self, block, passphrase):
        key = list(passphrase)
        l = list(block[:8])
        r = list(block[8:])
...
#        return encrypted
    ...
    def decrypt(self, ciphertext):
        ctr = open("./CONFIDENTIEL.xslx.enc","rb").read(16)
        print(bytes(ctr))
        #encrypted = b""
        passlist = open("/opt/SecLists/Passwords/rockyou.txt", "rb")
        i = 0
        seek = True
        while seek:
            candidate = passlist.readline()
            ...
            encryptedBlock = self.encryptBlock(
                ctr.to_bytes(16, 'big'))
            decryptedLine = bytes(
                self.xor(ciphertext, encryptedBlock)).hex()

            if str(decryptedLine)[:8] == "504b0304":
                seek = False
                print(f"passphrase is : {candidate}[:-1] ")
...
#    ciphertext = Encryptor(passphrase).encrypt(plaintext)
    ciphertext = open("CONFIDENTIEL.xslx.enc", "rb")
    plaintext = Encryptor(passphrase).decrypt(ciphertext)
    if args.output_file:
